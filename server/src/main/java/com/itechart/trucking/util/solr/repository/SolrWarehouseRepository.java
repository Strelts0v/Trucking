...
public interface SolrWarehouseRepository extends SolrCrudRepository<SolrWarehouseDocumentDto, String> {

    // @Query("name:*?0* AND country:*?1* AND city:*?2* AND street:*?3* AND house:*?4*")
    @Query("name:*?0* AND country:*?1* AND city:*?2* AND street:*?3*")
    List<SolrWarehouseDocumentDto> searchWarehouses(String name, String country, String city,
                                                    String street);
